...
class RewardModelTrainer(ABC):
    ...
    Args:
        ...
        max_select_samples (int, defaults to 5000): Maximum number of samples to select in active learning.
        score_type (str, defaults to "fisher"): Type of scoring function to use for active learning.
    """
    ...
    def __init__(
        ...
        # Initialize buffer for selected samples
        self.buffer = []
    ...
    def add_to_buffer(self, batch_data):
        """Add selected samples to buffer"""
        if not self.packing_samples:
            chosen_ids, c_mask, reject_ids, r_mask, margin = [x.detach().cpu() for x in batch_data]
            self.buffer.append((chosen_ids, c_mask, reject_ids, r_mask, margin))
        else:
            packed_input_ids, packed_attention_masks, packed_seq_lens, margin = batch_data
            packed_input_ids = packed_input_ids.detach().cpu()
            packed_attention_masks = packed_attention_masks.detach().cpu()
            self.buffer.append((packed_input_ids, packed_attention_masks, packed_seq_lens, margin))
    
    def retrain_on_buffer(self, step_bar):
        """Retrain on all samples in buffer"""
        self.strategy.print(f"\033[94mRetraining on buffer with {len(self.buffer)} batches\033[0m")
        ...
        for batch_data in self.buffer:
            if not self.packing_samples:
                chosen_ids, c_mask, reject_ids, r_mask, margin = [x.to(torch.cuda.current_device()) for x in batch_data]
            ...
            step_bar.set_postfix({"retrain_loss": loss.item(), "retrain_acc": acc})
        
        self.strategy.print("\n\033[94mCompleted retraining on buffer\033[0m")
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        # Counter for active learning samples
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                if not self.packing_samples:
                    ...
                    # print(packed_input_ids.shape)  # tensor: [1, sum(packed_seq_lens)]
                    # print(packed_attention_masks.shape)  #  tensor: [1, sum(packed_seq_lens)]
                    # print(sum(packed_seq_lens))  # list: length 16 (half chosen, half rejected), each element is the length of that input
                ...
                # Process selected samples from current batch
                if not self.packing_samples:
                    ...
                    # For packed samples, we need to handle the sequence lengths properly
                ...
                # Add selected samples to buffer
                if not self.packing_samples:
                    self.add_to_buffer((batch_chosen_ids, batch_c_mask, batch_reject_ids, batch_r_mask, batch_margin))
                else:
                    self.add_to_buffer((batch_input_ids, batch_attention_masks, batch_seq_lens, batch_margin))
                ...
                # Retrain on buffer every retrain_step steps
                if step % self.args.retrain_step == 0 and step > 0:
                    self.retrain_on_buffer(step_bar)
    ...
    def packed_samples_forward(self, model, packed_input_ids, packed_attention_masks, packed_seq_lens):
        ...
        chosen_rewards = all_values[:half_len]  # [8, 1]
        rejected_rewards = all_values[half_len:]  # [8, 1]
        # The order of chosen_rewards corresponds one-to-one with the prompts, where the same index indicates the same prompt.
        aux_loss = output.aux_loss if "aux_loss" in output else []
