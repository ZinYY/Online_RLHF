...
class RewardModelTrainer(ABC):
    def __init__(
        ...
        lambda_strategy: str = "linear",
        lambda_growth_rate: float = 20.0,
        hessian_decay: float = 1.0  
    ) -> None:
        super().__init__()
    ...
    def compute_hessian(self, params: List[nn.Parameter], loss: torch.Tensor) -> torch.Tensor:
        grads = torch.autograd.grad(loss, params, create_graph=True, retain_graph=True)
        flat_grad = torch.cat([g.reshape(-1) for g in grads if g is not None])
        
        hessian = []
        for i in range(len(flat_grad)):
            grad_i = torch.autograd.grad(flat_grad[i], params, retain_graph=True)
            hessian_row = torch.cat([g.reshape(-1) for g in grad_i if g is not None])
            hessian.append(hessian_row.detach())
        
        return torch.stack(hessian)
    
    def get_current_lambda(self) -> float:
        base_lambda = self.lambda_reg
        t = self.total_steps / self.total_T
        
        if self.lambda_strategy == "constant":
            return 1.0 if t * self.lambda_growth_rate >= 1.0 else base_lambda
        
        elif self.lambda_strategy == "linear":
            return min((1.0 - base_lambda) * t * self.lambda_growth_rate + base_lambda, 1.0)
        
        elif self.lambda_strategy == "log":
            return min(base_lambda * (1 + np.log(1 + self.lambda_growth_rate * t)), 1.0)
        
        elif self.lambda_strategy == "square":
            return min(base_lambda * (1 + (self.lambda_growth_rate * t) ** 2), 1.0)
        
        elif self.lambda_strategy == "cosine":
            return min(base_lambda * (2 - np.cos(self.lambda_growth_rate * np.pi * t)), 1.0)
        
        else:
            return base_lambda

    def newton_update(self, current_grad: torch.Tensor) -> torch.Tensor:
        current_lambda = self.get_current_lambda()
        
        if current_lambda < 1.0:
            total_hessian = current_lambda * torch.eye(self.head_dim, device=current_grad.device) + (1 - current_lambda) * self.cumulative_hessian
            
            original_dtype = current_grad.dtype
            
            current_grad = current_grad.float()
            total_hessian = total_hessian.float()
            
            try:
                update = torch.linalg.solve(total_hessian, -current_grad)
            
            except RuntimeError:
                total_hessian += 1e-4 * torch.eye(self.head_dim, device=current_grad.device)
                update = torch.linalg.solve(total_hessian, -current_grad)
            
            update = update.to(original_dtype)
        else:
            update = -current_grad
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                with torch.enable_grad():
                    grads = torch.autograd.grad(loss, head_params, create_graph=True, retain_graph=True)
                current_grad = torch.cat([g.reshape(-1) for g in grads if g is not None])
                
                if self.get_current_lambda() < 1.0:
                    current_hessian = self.compute_hessian(head_params, loss)
                    if self.cumulative_hessian is None:
                        self.head_dim = current_hessian.shape[0]
                        self.cumulative_hessian = current_hessian.detach()
                    else:
                        self.cumulative_hessian = self.hessian_decay * self.cumulative_hessian + current_hessian.detach()
                
                update = self.newton_update(current_grad)
                
                if self.args.verbose:
                    print('4.5 current_grad norm', torch.norm(current_grad, p=1))
                    print('4.5 update norm', torch.norm(update, p=1))
                
                # # dynamic scaling:
                # scale_factor = torch.norm(current_grad, p=1) / (torch.norm(update, p=1) + 1e-5)
                
                with torch.no_grad():
                    offset = 0
                    for param in head_params:
                        param_size = param.numel()
                        param_update = update[offset:offset + param_size].reshape(param.shape)
                        param.add_(param_update, alpha=args.learning_rate)
                        offset += param_size
