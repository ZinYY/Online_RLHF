...
def preprocess_data(data, input_template=None, input_key="input", output_key=None, apply_chat_template=None):
    if apply_chat_template:
        if output_key:
            ...
            if '/' in output_key:
                output_key = output_key.split('/')
                response_message = eval(str(data[output_key[0]]))[int(output_key[1])][output_key[2]]
            else:
        ...
        if output_key:
            if '/' in output_key:
                output_key = output_key.split('/')
                response = eval(str(data[output_key[0]]))[int(output_key[1])][output_key[2]]
            else:
