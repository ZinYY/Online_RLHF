...
# Using generated data to prepare a dataset
def make_dataset(args, objs):
    out = {}
    for obj in tqdm(objs, desc="Making dataset..."):
        input = obj["input"]
        output = obj["output"]
        reward = float(obj["reward"])
        
        if input not in out:
            out[input] = {"outputs": [output], "rewards": [reward]}
        else:
            out[input]["outputs"].append(output)
            out[input]["rewards"].append(reward)
    
    return [{"input": k, "outputs": v["outputs"], "rewards": v["rewards"]} for k, v in out.items()]
