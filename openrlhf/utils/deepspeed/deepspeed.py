...
class DeepspeedStrategy(ABC):
    ...
    def create_sgd_optimizer(self, model, **kwargs) -> Optimizer:
        if isinstance(model, Actor):
            model = model.model
        # use SGD Optimizer
        SGDOptimizer = torch.optim.SGD
        # optim_params = get_optimizer_grouped_parameters(model, kwargs["weight_decay"])
        optim = SGDOptimizer(model.parameters(), **kwargs)
        return optim
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.is_rank_0():
            ...
            if not state_dict_keys.issubset(output_state_dict_keys):
                warnings.warn(f"mismatch keys {output_state_dict_keys.symmetric_difference(state_dict_keys)}")
