        ...
        def forward(
            ...
            if return_hidden_states:
                if self.packing_samples:
                    ...
                    if len(eos_indices.shape) == 1:
                        eos_indices = eos_indices.unsqueeze(0)
                    # Expand indices to match hidden state dimensions
                    expanded_indices = eos_indices.unsqueeze(-1).expand(-1, -1, last_hidden_states.size(-1))
                    hidden_states = last_hidden_states.gather(dim=1, index=expanded_indices)
                else:
                    expanded_indices = eos_indices.unsqueeze(-1).expand(-1, -1, last_hidden_states.size(-1))
                    hidden_states = last_hidden_states.gather(dim=1, index=expanded_indices).squeeze(1)
