...
def train_reward_model_active_SGD(gpu_nodes, train_batch_size, args):
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port {get_random_port()} --module openrlhf.cli.active_train_rm_head \\
     --save_path ./checkpoint_ultraFB/llama3-8b-rm-active \\
     --save_steps -1 \\
     --logging_steps 1 \\
     --eval_steps 100 \\
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Active Learning Pipeline for LLaMA 3 with UltraFeedback')
    ...
    parser.add_argument('--wandb_key', type=str, default='XXXXX', help='Weights & Biases API key')
