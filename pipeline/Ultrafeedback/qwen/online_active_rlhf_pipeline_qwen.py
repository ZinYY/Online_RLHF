...
def train_reward_model_active_SGD(gpu_nodes, train_batch_size, args):
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port {get_random_port()} --module openrlhf.cli.active_train_rm_head \\
     --save_path ./checkpoint_ultraFB/qwen2.5-7b-rm-active \\
     --save_steps -1 \\
     --logging_steps 1 \\
     --eval_steps 100 \\
     ...
     --wandb_run_name qwen2.5-7b-rm-head-active-SGD-{args.score_type} \\
     --wandb_project openrlhf_RM_ultraFB_head_active
    """
...
def train_reward_model_active_hvp(gpu_nodes, train_batch_size, args):
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port {get_random_port()} --module openrlhf.cli.active_train_rm_head_hvp \\
     ...
     --packing_samples \\
     --use_wandb {args.wandb_key} \\
     --wandb_run_name qwen2.5-7b-rm-head-active-hvp-{args.score_type} \\
     --wandb_project openrlhf_RM_ultraFB_head_active
    """
    return run_command(cmd)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Active Learning Pipeline for Qwen 2.5 with UltraFeedback')
    ...
    os.makedirs("./checkpoint_ultraFB", exist_ok=True)
