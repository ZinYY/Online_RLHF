...
def train_reward_model_dataset0(current_t, total_T, gpu_nodes, train_batch_size, **kwargs):
    ...
    pretrain_path = "./checkpoint_mixture2/llama3-8b-sft" if current_t == 0 else f"./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3-8b-rm"
    
    # Choose dataset based on current iteration
    # if current_t == 0:
    dataset_args = """--dataset OpenRLHF/preference_dataset_mixture2_and_safe_pku \\"""
    # else:
    #     dataset_args = f"--dataset ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_current_t={current_t - 1}_output_PPO_dataset.json \\"
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port 27009 --module openrlhf.cli.online_train_rm_head \\
     --save_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3-8b-rm \\
     --save_steps -1 \\
     --logging_steps 1 \\
     --eval_steps -1 \\
     --train_batch_size 4 \\
     ...
     --flash_attn \\
     --packing_samples \\
     --wandb_project online_openrlhf_RM_mixture2 \\
     --total_T 1 \\
     --current_t 0 \\
     --t_length 1
    """
...
def train_reward_model(current_t, total_T, gpu_nodes, train_batch_size, **kwargs):
    ...
    for tt in range(1, current_t + 1):
        ...
        if current_t == 0:
            ...
            dataset_args = f"--dataset ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_current_t={tt - 1}_output_PPO_dataset.json \\"
...
def train_reward_model_hvp(current_t, total_T, gpu_nodes, train_batch_size, **kwargs):
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port 27009 --module openrlhf.cli.online_train_rm_head_hvp \\
     ...
     --flash_attn \\
     --packing_samples \\
     --wandb_project online_openrlhf_RM_mixture2_hvp \\
     --total_T 1 \\
     --current_t 0
    """
...
def train_dpo(gpu_nodes, train_batch_size, current_t, **kwargs):
    """Train using DPO"""
    pretrain_path = "meta-llama/Llama-3.2-1B-Instruct" if current_t == 0 else f"./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo"
    
    # Choose dataset based on current iteration
    if current_t == 0:
        dataset_args = """--dataset OpenRLHF/preference_dataset_mixture2_and_safe_pku \\"""
    else:
        dataset_args = f"--dataset ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_current_t={current_t - 1}_output_DPO_dataset.json \\"
    
    cmd = f"""
    export CUDA_VISIBLE_DEVICES={gpu_nodes}
    deepspeed --include=localhost:{gpu_nodes} --master_port 27012 --module openrlhf.cli.train_dpo \\
       --save_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo \\
       --save_steps -1 \\
       --logging_steps 1 \\
       --eval_steps -1 \\
       --train_batch_size 4 \\
       ...
       --flash_attn \\
       --load_checkpoint \\
       --wandb_project online_openrlhf_DPO_mixture2
    """
    return run_command(cmd)
...
def train_ppo(gpu_nodes, train_batch_size, current_t, total_T, **kwargs):
    ...
    pretrain_path = "meta-llama/Llama-3.2-1B-Instruct" if current_t == 0 else f"./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-rlhf_lora16_merged"
    cmd = f"""
    export CUDA_VISIBLE_DEVICES={gpu_nodes}
    deepspeed --include=localhost:{gpu_nodes} --master_port 27011 --module openrlhf.cli.train_ppo_online \
      --pretrain {pretrain_path} \
      --lora_rank 16 \
      --reward_pretrain ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3-8b-rm \
      --save_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-rlhf_lora16 \
      --save_steps -1 \
      --logging_steps 1 \
      --eval_steps -1 \
      --micro_train_batch_size 1 \
      ...
      --init_kl_coef 0.01 \
      --prompt_data OpenRLHF/preference_dataset_mixture2_and_safe_pku \
      --prompt_split train \
      --input_key messages \
      --apply_chat_template \
      --max_samples 10000 \
      ...
      --adam_offload \
      --flash_attn \
       --wandb_project online_openrlhf_PPO_mixture2 \
      --total_T {total_T} \
      --current_t {current_t}
    """
    ...
    merge_cmd = f"python merge_peft.py --peft_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-rlhf_lora16"
...
def generate_dpo_responses(gpu_nodes, tp_size, current_t, max_num_seqs, total_T, **kwargs):
    ...
    python -m openrlhf.cli.online_batch_inference \
       ...
       --pretrain ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo \
       --max_new_tokens 1024 \
       --prompt_max_len 1024 \
       --max_samples 10000 \
       ...
       --total_T {total_T} \
       --current_t {current_t} \
       --output_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_output_generate_vllm_current_t={current_t}.json
    """
    return run_command(cmd)
...
def generate_ppo_responses(gpu_nodes, tp_size, current_t, total_T, max_num_seqs, **kwargs):
    ...
    python -m openrlhf.cli.online_batch_inference \
       --eval_task generate_vllm \
       --pretrain ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-rlhf_lora16_merged \
       --max_new_tokens 1024 \
       --prompt_max_len 1024 \
       --max_samples 10000 \
       --dataset HuggingFaceH4/ultrafeedback_binarized \
       ...
       --total_T {total_T} \
       --current_t {current_t} \
       --output_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-rlhf_lora16_output_generate_vllm_current_t={current_t}.json
    """
    return run_command(cmd)
...
def evaluate_dpo_responses(gpu_nodes, current_t, total_T, **kwargs):
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port 27012 --module openrlhf.cli.online_batch_inference \
       ...
       --bf16 \
       --max_len 4096 \
       --dataset ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_output_generate_vllm_current_t={current_t}.json  \
       --dataset_probs 1.0 \
       --zero_stage 0 \
       ...
       --total_T 1 \
       --current_t 0 \
       --output_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_rm_oracle_current_t={current_t}.json
    """
    return run_command(cmd)
...
def evaluate_ppo_responses(gpu_nodes, current_t, total_T, **kwargs):
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port 27012 --module openrlhf.cli.online_batch_inference \
       ...
       --bf16 \
       --max_len 4096 \
       --dataset ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-rlhf_lora16_output_generate_vllm_current_t={current_t}.json  \
       --dataset_probs 1.0 \
       --zero_stage 0 \
       --post_processor make_dataset \
       --micro_batch_size 1 \
       --total_T 1 \
       --current_t 0 \
       --output_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_rm_oracle_current_t={current_t}.json
    """
    return run_command(cmd)
...
def reward_dataset_ppo(gpu_nodes, current_t, total_T, **kwargs):
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port 27012 --module openrlhf.cli.online_batch_inference \
       --eval_task rm \
       --pretrain ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3-8b-rm \
       --bf16 \
       --max_len 1024 \
       --dataset ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-rlhf_lora16_output_generate_vllm_current_t={current_t}.json  \
       --dataset_probs 1.0 \
       ...
       --total_T 1 \
       --current_t 0 \
       --output_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_rm_evaluated_current_t={current_t}.json
    """
    return run_command(cmd)
...
def reward_dataset_dpo(gpu_nodes, current_t, total_T, **kwargs):
    ...
    deepspeed --include=localhost:{gpu_nodes} --master_port 27012 --module openrlhf.cli.online_batch_inference \
       ...
       --total_T 1 \
       --current_t 0 \
       --output_path ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_rm_evaluated_current_t={current_t}.json
    """
    return run_command(cmd)
...
def collect_dpo_results(current_t, **kwargs):
    ...
    cmd = f"""python -m openrlhf.utils.collect_results \
        --score_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_current_t={current_t}_output_DPO_dataset.json"""
    ...
    try:
        ...
        results_file = f"./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/mean_scores.txt"
...
def collect_results(current_t, **kwargs):
    """Collect and save results from the current iteration"""
    cmd = f"""python -m openrlhf.utils.collect_results \
        --score_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_current_t={current_t}_output_PPO_dataset.json"""
...
def generate_final_dataset_dpo(current_t, **kwargs):
    ...
    compare_cmd = f"""python -m openrlhf.utils.compare_rewarded_outputs \
        --eval_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_rm_evaluated_current_t={current_t}.json \
        --true_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_rm_oracle_current_t={current_t}.json \
        --output_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_rm_current_t={current_t}_comparison.json"""
    ...
    convert_cmd = f"""python -m openrlhf.utils.convert_to_dataset \
        --eval_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_rm_evaluated_current_t={current_t}.json \
        --true_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_rm_oracle_current_t={current_t}.json \
        --output_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-dpo_current_t={current_t}_output_DPO_dataset.json \
...
def generate_final_dataset(current_t, **kwargs):
    ...
    # Compare rewards
    compare_cmd = f"""python -m openrlhf.utils.compare_rewarded_outputs \
        --eval_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_rm_evaluated_current_t={current_t}.json \
        --true_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_rm_oracle_current_t={current_t}.json \
        --output_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_rm_current_t={current_t}_comparison.json"""
    ...
    convert_cmd = f"""python -m openrlhf.utils.convert_to_dataset \
        --eval_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_rm_evaluated_current_t={current_t}.json \
        --true_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_rm_oracle_current_t={current_t}.json \
        --output_file ./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}/llama3.2-1b-ppo_current_t={current_t}_output_PPO_dataset.json \
        --strategy {args.rm_strategy}"""
...
def run_rlhf_recipe(args, rlhf_recipe, step_functions):
    ...
    checkpoint_dir = f"./checkpoint_online_mixture2_llama3_{args.method}_{args.rm_type}_{args.rm_strategy}"
...
def main():
    ...
    parser.add_argument('--stop_t', type=int, default=50, help='Stopping iteration')
